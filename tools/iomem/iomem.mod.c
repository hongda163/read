#include <linux/module.h>
#include <linux/vermagic.h>
#include <linux/compiler.h>

MODULE_INFO(vermagic, VERMAGIC_STRING);
MODULE_INFO(name, KBUILD_MODNAME);

__visible struct module __this_module
__attribute__((section(".gnu.linkonce.this_module"))) = {
	.name = KBUILD_MODNAME,
	.init = init_module,
#ifdef CONFIG_MODULE_UNLOAD
	.exit = cleanup_module,
#endif
	.arch = MODULE_ARCH_INIT,
};

#ifdef RETPOLINE
MODULE_INFO(retpoline, "Y");
#endif

static const struct modversion_info ____versions[]
__used
__attribute__((section("__versions"))) = {
	{ 0x36150fb8, __VMLINUX_SYMBOL_STR(module_layout) },
	{ 0xede7ecfb, __VMLINUX_SYMBOL_STR(param_ops_int) },
	{ 0x8d15114a, __VMLINUX_SYMBOL_STR(__release_region) },
	{ 0x45a55ec8, __VMLINUX_SYMBOL_STR(__iounmap) },
	{ 0x7485e15e, __VMLINUX_SYMBOL_STR(unregister_chrdev_region) },
	{ 0x37a0cba, __VMLINUX_SYMBOL_STR(kfree) },
	{ 0xe619bcc9, __VMLINUX_SYMBOL_STR(cdev_del) },
	{ 0xdcb7857, __VMLINUX_SYMBOL_STR(kmalloc_caches) },
	{ 0xa5873a99, __VMLINUX_SYMBOL_STR(cpu_hwcaps) },
	{ 0xc364ae22, __VMLINUX_SYMBOL_STR(iomem_resource) },
	{ 0x208bb3a, __VMLINUX_SYMBOL_STR(cdev_add) },
	{ 0xc09d1368, __VMLINUX_SYMBOL_STR(cdev_init) },
	{ 0xa7707f5c, __VMLINUX_SYMBOL_STR(kmem_cache_alloc_trace) },
	{ 0x29537c9e, __VMLINUX_SYMBOL_STR(alloc_chrdev_region) },
	{ 0xd8e484f0, __VMLINUX_SYMBOL_STR(register_chrdev_region) },
	{ 0xf24b3dfe, __VMLINUX_SYMBOL_STR(__ioremap) },
	{ 0xfa37a7ca, __VMLINUX_SYMBOL_STR(cpu_hwcap_keys) },
	{ 0x9ee37840, __VMLINUX_SYMBOL_STR(arm64_const_caps_ready) },
	{ 0x5e25804, __VMLINUX_SYMBOL_STR(__request_region) },
	{ 0x27e1a049, __VMLINUX_SYMBOL_STR(printk) },
	{ 0x1fdc7df2, __VMLINUX_SYMBOL_STR(_mcount) },
};

static const char __module_depends[]
__used
__attribute__((section(".modinfo"))) =
"depends=";


MODULE_INFO(srcversion, "40FF5B9CFDB49DA09400835");
